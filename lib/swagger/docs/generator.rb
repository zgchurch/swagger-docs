      ...
      class << self
        ...
        def write_doc(api_version, config)
          ...
          paths.each do |path|
            ...
            resources[:apis] << {path: "#{config[:api_file_path].gsub('public/', '')}#{Config.transform_path(trim_leading_slash(debased_path))}.{format}", description: klass.swagger_config[:description]}
